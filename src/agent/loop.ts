/**
 * One turn of the agent: send the messages, stream the answer, report what came back.
 *
 * Phase 1 is a single generation with no tools. Phase 2 wraps this in the constrained
 * tool loop; the shape here — an async function that emits events through a callback and
 * returns the assistant message — is what that loop will call per iteration.
 */
